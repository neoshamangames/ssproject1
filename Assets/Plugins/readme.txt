...
Android native plugin to prevent cheating by changing device time in settings. 
...
### Usage
 ...
 1. Plugins/Android folder contains java lib: `unbiasedtime.jar`. 
    It should be included automatically in the generated apk file. 
...
Simple demo included (works only on real Android device). It have two timers: one naive using `DateTime.Now` and the other 
is using `UnbiasedTime.Instance.Now()`. Timers are implemented by remembering timestamp of timespan end in PlayerPrefs 
so they keep counting even if app is backgrounded or closed. Launch it on your Android device, tap +60 seconds several times, then go to
