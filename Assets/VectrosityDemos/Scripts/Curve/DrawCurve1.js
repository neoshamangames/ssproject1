...
private var curvePoints : Vector3[];
...
function Start () {
	...
	curvePoints[0] = Vector3(Screen.width*.25, Screen.height*.25, 0);
	curvePoints[1] = Vector3(Screen.width*.125, Screen.height*.5, 0);
	curvePoints[2] = Vector3(Screen.width-Screen.width*.25, Screen.height-Screen.height*.25, 0);
	curvePoints[3] = Vector3(Screen.width-Screen.width*.125, Screen.height*.5, 0);
...
function OnGUI () {
	...
	drawEnd = GUI.HorizontalSlider(Rect(200, 80, 150, 30), drawEnd, 0, 200);
	if (GUI.changed) {
		ChangeSegments();
	}
...
function AddPoint () {
	...
	if (line.points3.Length + segments > 16384) return;
	
	System.Array.Resize.<Vector3>(linePoints, (segments+1) * ++numberOfCurves);
	System.Array.Resize.<Vector3>(curvePoints, numberOfCurves*4);
...
function ChangeSegments () {
	...
	linePoints = new Vector3[(segments+1) * numberOfCurves];
	line.Resize(linePoints);
